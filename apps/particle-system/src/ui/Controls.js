import { CONFIG_STORAGE_KEY, LEGACY_CONFIG_COOKIE_NAME, presets, } from "../simulation/config.js";
export class Controls {
    ...
    setInteractionRadiusMax(max) {
        const input = this.values.get("interactionRadius");
        if (!(input instanceof HTMLInputElement))
            return;
        const nextMax = Math.max(12, Math.round(max));
    ...
    createPresetSection() {
        ...
        select.addEventListener("change", () => {
            ...
            this.onChange({ ...preset, seed: this.config.seed }, true);
        });
    ...
    select(key, labelText, options) {
        ...
        select.addEventListener("change", () => {
            this.onChange({ ...this.config, [key]: select.value }, true);
        });

import {
    ...
    PREVIOUS_CONFIG_STORAGE_KEY,
    presets,
    TOUCH_MODE_OPTIONS,
} from "../simulation/config.js";

const TOUCH_SETTING_KEYS = [
    "touchEnabled",
    "touchModePrimary",
    "touchModeSecondary",
...
export class Controls {
    ...
    updateStats(stats, particleCount) {
      ...
      <dl>
        ...
        <dt>Pair checks</dt><dd>${stats.diagnostics.pairChecks}</dd>
        <dt>Candidates</dt><dd>${stats.diagnostics.neighborCandidates}</dd>
        <dt>Active touches</dt><dd>${stats.diagnostics.touchPoints}</dd>
        <dt>Touch effects</dt><dd>${stats.diagnostics.touchApplications}</dd>
      </dl>
    `;
    }
    ...
    createPresetSection() {
        ...
        select.addEventListener("change", () => {
            ...
            const touchSettings = Object.fromEntries(
                TOUCH_SETTING_KEYS.map((key) => [key, this.config[key]]),
            );
            this.onChange({ ...preset, ...touchSettings, seed: this.config.seed }, true);
        });
    ...
    createChamberSection() {
        ...
        for (const control of chamberControls)
            fragment.append(this.range(control));
    ...
    createTouchSection() {
        ...
        for (const control of touchSharedControls)
            fragment.append(this.range(control));
    ...
    createRenderingSection() {
        ...
        for (const control of renderingControls)
            fragment.append(this.range(control));
...
const touchSharedControls = [
    { key: "touchRadius", label: "Touch radius", min: 30, max: 360, step: 5 },
...
export function loadStoredConfig() {
    try {
        ...
        const previous = localStorage.getItem(PREVIOUS_CONFIG_STORAGE_KEY);
        if (previous) {
            const parsed = JSON.parse(previous);
            localStorage.setItem(CONFIG_STORAGE_KEY, JSON.stringify(parsed));
            localStorage.removeItem(PREVIOUS_CONFIG_STORAGE_KEY);
            return parsed;
        }

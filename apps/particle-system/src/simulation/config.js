...
const PHYSICS_ENGINES = new Set(PHYSICS_ENGINE_OPTIONS.map(({ value }) => value));
...
export const defaultConfig = {
  ...
  backgroundFade: 0.3,
  physicsEngine: "cpu",
  gpuFieldResolution: 128,
  gpuFieldForceScale: 1,
  touchEnabled: false,
...
export function sanitizeConfig(input, interactionRadiusMax = 4000) {
  ...
  return {
    ...
    physicsEngine: sanitizePhysicsEngine(input.physicsEngine),
    gpuFieldResolution: clampGridSize(input.gpuFieldResolution ?? defaultConfig.gpuFieldResolution),
    gpuFieldForceScale: clampNumber(input.gpuFieldForceScale ?? defaultConfig.gpuFieldForceScale, 0.1, 3),
...
function sanitizePhysicsEngine(value) {
  return PHYSICS_ENGINES.has(value) ? value : defaultConfig.physicsEngine;
}
...
function clampGridSize(value) {
  const clamped = clampInt(value, 64, 256);
